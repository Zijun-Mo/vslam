...
def vggt_resize(img, depth, new_size=(392, 518)):
    ...
    return (
        np.ascontiguousarray(resized_img),
        np.ascontiguousarray(resized_depth),
        (scale_w, scale_h, half_crop_w, half_crop_h),
    )
...
def load_7scenes(dataset, W, H, calib):
    ...
    for i, (rgb_file, depth_file, pose_file) in enumerate(
        zip(rgb_files, depth_files, pose_files)
    ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset",
        default=default_root / "datasets/7-scenes/office",
        type=pathlib.Path,
    )
    parser.add_argument(
        "--gt",
        default=default_root / "datasets/groundtruths/7-scenes/office.txt",
        type=pathlib.Path,
    )
    parser.add_argument(
        "--est", default=default_root / "logs/office.txt", type=pathlib.Path
    )
    ...
    traj_est = file_interface.read_tum_trajectory_file(str(args.est))
    matches = sync.matching_time_indices(traj_ref.timestamps, traj_est.timestamps)

    traj_ref, traj_est = sync.associate_trajectories(traj_ref, traj_est)

    traj_est_aligned = copy.deepcopy(traj_est)
    r_a, t_a, s = traj_est_aligned.align(
    ...
    temp_path = str(args.est).replace(".txt", "_logs/")
