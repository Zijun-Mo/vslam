...
def as_SE3(T):
    """Lightweight fallback: wrap an array-like [tx,ty,tz,rx,ry,rz] into lietorch.SE3."""
    if isinstance(T, lietorch.SE3):
        return T
    tensor = torch.as_tensor(T).view(1, -1)
    return lietorch.SE3(tensor)


def lietorch_to_mat(T, return_scale=False):
    """Convert lietorch.SE3 to 4x4 numpy matrix. Scale defaults to 1.0."""
    if not isinstance(T, lietorch.SE3):
        T = as_SE3(T)
    mat = T.matrix().squeeze(0).cpu().numpy()
    scale = 1.0
    return (mat, scale) if return_scale else mat
