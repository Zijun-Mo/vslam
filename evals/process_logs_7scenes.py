...
parser = argparse.ArgumentParser(description="Process 7-Scenes results")
parser.add_argument("--submap_size", type=str, default="32", help="submap size to use")
args = parser.parse_args()

log_path = Path.cwd() / ("logs/" f"7scenes_results_w{args.submap_size}.txt")
...
for run in sorted(df["Run"].unique()):
    ...
    for _, row in run_df.iterrows():
        ...
        chamfer = row["Chamfer"]
        print(
            f"{scene}: RMSE={rmse:.4f}, Acc={acc:.4f}, Comp={comp:.4f}, Chamfer={chamfer:.4f}"
        )
