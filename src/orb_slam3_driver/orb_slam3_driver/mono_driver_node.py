...
class MonoDriver(Node):
    def __init__(self, node_name = "mono_py_node"):
        ...
        # Updated to match the current workspace
        self.home_dir = str(Path.home()) + "/vslam/src/orb_slam3_driver" 
        ...
        if self.image_seq != "NULL":
            ...
            print("No image sequence provided. Waiting for parameters...")
        ...
        # Updated to match the topic expected by orb_slam3_tracking
        self.pub_img_to_agent_name = "camera/image_raw" 
        self.pub_timestep_to_agent_name = "/mono_py_driver/timestep_msg"
        self.send_config = False # Set False as we don't need handshake for the new split architecture
        ...
        self.publish_img_msg_ = self.create_publisher(Image, self.pub_img_to_agent_name, 10)
    ...
    def get_image_dataset_asl(self, exp_dir, agent_name = "mav0"):
        ...
        if not os.path.exists(imgz_file_dir):
            print(f"Error: Directory {imgz_file_dir} does not exist.")
            return "", [], []
    ...
    def run_py_node(self, idx, imgz_name):
        ...
        if image is None:
            print(f"Failed to load image: {img_look_up_path}")
            return
        ...
        # Also encode timestamp in nanoseconds in the header for ORB-SLAM3 if needed, 
        # but standard ROS header stamp is usually preferred.
        # The original code sent a separate float message.
        ...
        try:
            ...
            print(f"Published frame {self.frame_id}: {imgz_name}")
...
def main(args = None):
    ...
    # Disabled handshake for now as we are using the new split architecture
    ...
    end_idx = n.end_frame if n.end_frame != -1 else None
    
    for idx, imgz_name in enumerate(n.imgz_seqz[n.start_frame:end_idx]):
        try:
            rclpy.spin_once(n, timeout_sec=0.01) # Blocking we need a non blocking take care of callbacks
            n.run_py_node(idx, imgz_name)
            # rate.sleep() 
            time.sleep(0.05) # Sleep for 50ms (20Hz) manually to avoid clock issues
