...
public:
    ...
    //* Constructor
    System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor, const bool bUseViewer = true, const int initFr = 0, const string &strSequence = std::string(), const bool bStartThreads = true);
    ...
    // Pointers to modules (Made public for ROS 2 split architecture)
    Tracking* mpTracker;
...
private:
    ...
    //Map* mpMap;
    // Atlas* mpAtlas; // Moved to public

    // Tracker. It receives a frame and computes the associated camera pose.
    // It also decides when to insert a new keyframe, create some new MapPoints and
    // performs relocalization if tracking fails.
    // Tracking* mpTracker; // Moved to public

    // Local Mapper. It manages the local map and performs local bundle adjustment.
    // LocalMapping* mpLocalMapper; // Moved to public

    // Loop Closer. It searches loops with every new keyframe. If there is a loop it performs
    // a pose graph optimization and full bundle adjustment (in a new thread) afterwards.
    // LoopClosing* mpLoopCloser; // Moved to public
